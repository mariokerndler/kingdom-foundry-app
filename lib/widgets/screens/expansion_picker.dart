...
class _ExpansionPickerTabState extends ConsumerState<ExpansionPickerTab>
  ...
  Widget build(BuildContext context) {
    ...
    return availableAsync.when(
      ...
      error:   (e, _) =>
          Center(child: Text('Error loading expansions: $e')),
      data: (available) {
        ...
        final totalKingdom = statsAsync.whenOrNull(
          data: (s) => owned.fold<int>(
              0, (sum, e) => sum + (s[e]?.kingdom ?? 0)),
        );
...
class _GroupedList extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final large = expansions
        .where((e) => (stats[e]?.kingdom ?? 0) >= 20)
        .toList();
    final small = expansions
        .where((e) => (stats[e]?.kingdom ?? 0) < 20)
        .toList();
